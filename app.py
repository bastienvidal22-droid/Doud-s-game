...
def save_playlist(new_playlist):
    # On écrase la donnée sur le cloud
...
if not st.session_state.game_started:
    st.info(f"Playlist collaborative en ligne. Déjà {len(playlist)} titres !")
    ...
    with st.form("ajout"):
        ...
        if st.form_submit_button("Envoyer au Cloud 🚀"):
            if name and link and (vid := extract_video_id(link)):
                ...
                current = load_playlist()
                # On gère le cas où le jsonbin contient juste {"test": "ok"} au début
